...
Page({
  ...
  onLoad: function(options) {
    ...
    var that = this,
      countdownid = options.scene ? options.scene.split('-')[0] : options.id;
      ...
      wx.request({
        ...
        success: function(result) {
          ...
          if (result.success) {
            ...
            shopScore.ProductAndDescription = r.Shop.ProductAndDescription.toFixed(
              2
            );
            shopScore.SellerDeliverySpeed = r.Shop.SellerDeliverySpeed.toFixed(
              2
            );
            shopScore.SellerServiceAttitude = r.Shop.SellerServiceAttitude.toFixed(
              2
            );
            that.setData({
              ShopScore: shopScore,
              ...
              Coupons: r.Coupons,
              ShowPrice:
                r.MaxSalePrice == r.MinSalePrice
                  ? r.MinSalePrice
                  : r.MinSalePrice + "～" + r.MaxSalePrice,
  ...
  onSharePoster: function() {
    ...
    app.getOpenId(function(openId) {
      ...
      wx.request({
        url: app.getUrl(
          `${app.globalData.getLimitShareTheGoods}?openId=${openId}&productId=${that.data.CountDownId}`
        ),
        method: "POST",
        success: function(result) {
          result = result.data;
              ...
              success: res => {
                ...
                wx.getImageInfo({
                  ...
                  success: res => {
                    ...
                    wx.getImageInfo({
                      src: app.getUrl(
                        `${app.globalData.getQrCode}?openId=${openId}&scene=${that.data.CountDownId}-${distributorId}&page=pages/countdowndetail/countdowndetail`
                      ),
                      success: res => {
                        shareObj.qrcode = res.path;

                        util.createdShareImg(shareObj, 'xianshigou');
                        setTimeout(() => {
  ...
  onShareAppMessage: function() {
    ...
    var that = this;
    var imageUrl =
      this.data.ProductImgs.length > 0 ? this.data.ProductImgs[0] : "";
    var path =
      "/pages/countdowndetail/countdowndetail?id=" + this.data.CountDownId;
    if (userInfo.IsDistributor) {
    ...
    return {
      ...
      path: "/pages/home/home?sharePage=" + encodeURIComponent(path),
      imageUrl: imageUrl,
      success: function(res) {},
      fail: function(res) {
        // 转发失败
  ...
  clickCouponList: function(e) {
    const that = this;
    if (
      that.data.Coupons != undefined &&
      that.data.Coupons != null &&
      that.data.Coupons != "" &&
      that.data.Coupons.length > 0
    ) {
      this.setData({
        backShow: "",
        couponShow: ""
  ...
  changeAmount: function(e) {
    ...
    var stock =
      this.data.MaxCount > this.data.skuStock
        ? this.data.skuStock
        : this.data.MaxCount;
    ...
    if (isNaN(amount) || amount > stock || amount <= 0) {
      ...
      app.showErrorModal(
        "请输入正确的数量,不能大于最大抢购数量和商品库存或者小于等于0"
      );
      return;
    } else {
      this.setData({
  ...
  commitBuy: function(e) {
    ...
    app.getOpenId(function(openid) {
      wx.navigateTo({
        url:
          "../submitorder/submitorder?productsku=" +
          skuid +
          "&buyamount=" +
          amount +
          "&frompage=1&countdownid=" +
          countdownid
      });
    });
  },
